...
import styles from "./UKDocumentsForm.module.css";

export default function UKDocumentsForm() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`/api/editukdoc/${id}`, {
        method: "DELETE",
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingId ? `/api/editukdoc/${editingId}` : "/api/ukdoc";
    ...
    <div className={styles.container}>
      <div className={styles.formWrapper}>
        <h2 className={styles.title}>
          {editingId ? "Edit UK Document" : "Submit UK Documents"}
        </h2>
